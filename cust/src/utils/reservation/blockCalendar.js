export const blockCalendar = ({ date }) => {
  ...
  const nextDate = currentDate.setDate(currentDate.getDate() - 1); // 내일 날짜
  ...
  return date < minSelectableDate || date > maxSelectableDate || date < nextDate; // 범위에서 벗어나고 현재 일 보다 이전은 선택 불가능
};